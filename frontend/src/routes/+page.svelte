    ...
    $:{
        if(value>0){
            ...
            prevTimeoutID = setTimeout(()=>{
               ...
               ws = new WebSocket(`ws://192.168.101.3:8000/connect?freq=${value}`)
...
<div class="w-screen h-screen flex justify-center items-center bg-purple-900">
    <div class="w-3 h-16 bg-black absolute bottom-[85%] right-[%] border-t-[1rem] {blinker?"border-t-red-500":"border-t-black"} rounded-t-md"/>
    <div class="bg-purple-700 grid grid-rows-6 w-[90%] md:w-[50%] max-w-[25rem] h-[70%] shadow-md">
        <input max="1000" min="0" bind:value class="row-span-2 px-4 text-2xl outline-none min-w-0" type="number">
        <div on:mousedown={dragstart} on:mouseleave={()=>dragging=false} on:mousemove={mousemoving} on:mouseup={dragend} class="bg-purple-400 cursor-pointer relative row-start-4 row-span-2 m-auto h-48 w-48 max-w-[15rem] shadow-md flex items-center justify-center rounded-[50%]">
            <p class="z-10 text-2xl select-none text-white font-bold">
                {value} Hz
            </p>
            <div style="transform: rotate({roundedValue}deg);" class="absolute w-2 bg-red-500 origin-bottom h-[50%] border-b-[2rem] border-b-purple-400 top-0"/>
        </div>
    </div>
